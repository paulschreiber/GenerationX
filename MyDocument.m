...
- (void)windowControllerDidLoadNib:(NSWindowController *) aController
{
	...
	[[indiTable tableColumnWithIdentifier: @"sex"] setDataCell: [[NSImageCell alloc] init]];
	[indiTable selectRow: 0 byExtendingSelection: NO];
	currentIndi = [indiDataSource indiAtIndex: 0];
	if( [indiDataSource numberTotal] == 1 )
	  [indiMessageText setStringValue: [NSString stringWithFormat: @"%@ person", [[NSNumber numberWithInt: [indiDataSource numberTotal]] stringValue]]];
	else
	  [indiMessageText setStringValue: [NSString stringWithFormat: @"%@ people", [[NSNumber numberWithInt: [indiDataSource numberTotal]] stringValue]]];
	...
	if( [famDataSource numberTotal] == 1 )
	  [famMessageText setStringValue: [NSString stringWithFormat: @"%@ family", [[NSNumber numberWithInt: [famDataSource numberTotal]] stringValue]]];
	else
	  [famMessageText setStringValue: [NSString stringWithFormat: @"%@ families", [[NSNumber numberWithInt: [famDataSource numberTotal]] stringValue]]];
...
// the requested INDI is not visible in the filtered list
  int index = [indiDataSource indexOfIndi: i];

	[indiTable selectRow: index byExtendingSelection: NO];
...
// need more elegant handling of the case where
// the requested INDI is not visible in the filtered list
  int index = [famDataSource indexOfFam: f];

	[famTable selectRow: index byExtendingSelection: NO];
...
- (void) handleAddIndi: (id) sender
{
  ...
  while( [ged recordWithLabel: label] )		
	...
	new = [[INDI alloc] init: 0 : @"INDI" : label];
...
- (void) handleFilterIndividuals: (id) sender
{
  [indiDataSource filterWithString: [indiSearchField stringValue]];
	[indiTable reloadData];
	[indiMessageText setStringValue: [NSString stringWithFormat: @"%@ of %@ people", [[NSNumber numberWithInt: [indiDataSource numberDisplayed]] stringValue], [[NSNumber numberWithInt: [indiDataSource numberTotal]] stringValue]]];
}
...
- (void) handleAddFam: (id) sender
{
  ...
  while( [ged recordWithLabel: label] )		
	...
	new = [[FAM alloc] init: 0 : @"FAM" : label];
...
- (void) handleFilterFamilies: (id) sender
{
  [famDataSource filterWithString: [famSearchField stringValue]];
	[famTable reloadData];
	[famMessageText setStringValue: [NSString stringWithFormat: @"%@ of %@ families", [[NSNumber numberWithInt: [famDataSource numberDisplayed]] stringValue], [[NSNumber numberWithInt: [famDataSource numberTotal]] stringValue]]];
}
...
- (void) handleAddSource: (id) sender
{
  ...
  while( [ged recordWithLabel: label] )		
	...
	new = [[GCField alloc] init: 0 : @"SOUR" : label];
