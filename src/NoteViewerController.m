...
{
	GCField* gc_tmp;
	NSMutableString* title = [NSMutableString stringWithString: @"Notes for "];
	NSUInteger i = 0;
	field = my_field;
...
- (void) handleAddNote: (id) sender
{
	if ( [note_outline selectedRow] == -1 ) {
		[field addSubfield: @"NOTE": @""];
	} else if ( [[note_outline itemAtRow: [note_outline selectedRow]] isEvent] ) {
		[[note_outline itemAtRow: [note_outline selectedRow]] addSubfield: @"NOTE": @""];
	} else {
		return;
	}
