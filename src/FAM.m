...
- (NSString*) textSummary: (id) my_ged;
{
	...
	if ( [[self subfieldWithType: @"MARR"] subfieldWithType: @"DATE"] ) {
		[result appendString: [[self subfieldWithType: @"MARR"] valueOfSubfieldWithType: @"DATE"]];
	}
...
- (void) sortChildren: (id) g
{
	...
	for ( i = 0; i < [children count]; i++ ) {
		...
		[subfields addObject: [[[GCField alloc] init: 1 : @"CHIL" : [f fieldValue]] autorelease]];
	}
}
...
- (NSComparisonResult) compareMarriageDate: (id) a
{
	...
	} else if ( [[self subfieldWithType: @"MARR"] valueOfSubfieldWithType: @"DATE"] ) {
		return NSOrderedAscending;
	} else if ( [[a subfieldWithType: @"MARR"] valueOfSubfieldWithType: @"DATE"] ) {
		return NSOrderedDescending;
	} else {
		return NSOrderedSame;
	}
