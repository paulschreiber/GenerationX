...
- (NSString*) lifespan
{
	...
	if ( birt_year > 1000 && birt_year < 9999 ) {
		[result appendString:
		 [[NSNumber numberWithInteger: birt_year] stringValue]];
	}
	[result appendString: @"-"];
	if ( deat_year > 1000 && deat_year < 9999 ) {
		[result appendString:
		 [[NSNumber numberWithInteger: deat_year] stringValue]];
	}
	[result appendString: @")"];
...
{
	NSMutableString* result = [NSMutableString stringWithCapacity:1];
	NSMutableString* prefix = [NSMutableString stringWithCapacity:1];
	NSArray* spice = [self spouseFamilies: my_ged];
	NSArray* children;
	NSString* tmp_str;
	INDI* tmp_indi;
	...
	[result appendString: @"("];
	[result appendString: [[NSNumber numberWithInteger: my_level] stringValue]];
	[result appendString: @") "];
	...
	for ( j = 0; j < [spice count]; j++ ) {
		if ( j > 0 ) {
			...
			[result appendString: [[NSNumber numberWithInteger: (j + 1)] stringValue]];
...
- (NSString*) ancestorsReportText: (id) my_ged: (NSString*) my_prefix
{
	...
	NSMutableString* result = [NSMutableString stringWithCapacity:1];
	//  NSMutableString* prefix = [NSMutableString stringWithCapacity:1];
	NSMutableString* prefix2 = [NSMutableString stringWithString: my_prefix];
	if ( [prefix2 length] > 2 ) {
...
- (NSComparisonResult) compareBirthdays: (id) a
{
	...
	} else if ( [[self subfieldWithType: @"BIRT"] valueOfSubfieldWithType: @"DATE"] ) {
		return NSOrderedAscending;
	} else if ( [[a subfieldWithType: @"BIRT"] valueOfSubfieldWithType: @"DATE"] ) {
...
- (NSComparisonResult) compareDeathDates: (id) a
{
		...
		&& (date2_str = [[a subfieldWithType: @"DEAT"] valueOfSubfieldWithType: @"DATE"]) )
	{
		...
		date2 = [NSDate dateWithNaturalLanguageString: date2_str];
		return [date1 compare: date2];
	} else if ( [[self subfieldWithType: @"DEAT"] valueOfSubfieldWithType: @"DATE"]) {
		return NSOrderedAscending;
	} else if ( [[a subfieldWithType: @"DEAT"] valueOfSubfieldWithType: @"DATE"]) {
		return NSOrderedDescending;
	} else {
		return NSOrderedSame;
	}
