...
- (GCFile*)init
{
  ...
  NSMutableString* todays_date_str = [[NSMutableString alloc] init];
  NSMutableString* subm_label = [[NSMutableString alloc] init];
  ...
  [todays_date_str setString: [[NSNumber numberWithInt: [todays_date dayOfMonth]] stringValue]];
  [todays_date_str appendString: @" "];
  [todays_date_str appendString: [month_array objectAtIndex: ([todays_date monthOfYear] - 1)]];
  [todays_date_str appendString: @" "];
  [todays_date_str appendString: [[NSNumber numberWithInt: [todays_date yearOfCommonEra]] stringValue]];
...
- (id) addRecord: (NSString*) my_type : (NSString*) my_value
{
  ...
  {
    INDI* record = [[INDI alloc] init: 0 : my_type : my_value];
    [individuals addObject: record];
    num_indi++;
    return record;
  }
  else if( [my_type isEqual: @"FAM"] )
  {
    FAM* record = [[FAM alloc] init: 0 : my_type : my_value];
...
- (GCField*)removeRecord: (GCField*) my_field
{
  ...
  if( [[my_field fieldType] isEqual: @"INDI"] )
  {
    ...
    for( i = 0; i < num_fam; i++ )
    {
      ...
      [tmp removeSubfieldWithType: @"WIFE" Value: [record fieldValue]];
      [tmp removeSubfieldWithType: @"CHIL" Value: [record fieldValue]];
      /*
    ...
    [individuals removeObject: record];
    num_indi--;
    return record;
  }
  else if( [[my_field fieldType] isEqual: @"FAM"] )
  {
    ...
    for( i = 0; i < num_indi; i++ )
    {
      ...
      [tmp removeSubfieldWithType: @"FAMC" Value: [record fieldValue]];
    }
    
    // now remove the record
    [families removeObject: record];
    num_fam--;
    return record;
  }
  else
...
- (BOOL)loadData
{
  ...
  GCField* current_record = [GCField alloc];
  GCField* working_field = [GCField alloc];
  int i;
  ...
  while( [file_scanner scanUpToCharactersFromSet:
           ...
           intoString: &line] )
  {
    line_scanner = [line_scanner initWithString: line];
...
- (BOOL) saveToFile
{
  ...
  NSMutableString* out_text =
    [[NSMutableString alloc] initWithString:
    [[self recordWithLabel: @"HEAD"] dataForFile]];
  NSMutableString* todays_date_str = [[NSMutableString alloc] init];
  NSArray* month_array = [NSArray arrayWithObjects:
    @"JAN", @"FEB", @"MAR", @"APR", @"MAY", @"JUN",
    @"JUL", @"AUG", @"SEP", @"OCT", @"NOV", @"DEC", nil];
